...
class OrderFormInfo(FlaskForm):
    scheduled_pickup_date = DateTimeLocalField('Schedule Pickup', format='%Y-%m-%dT%H:%M',
                                               validators=(DataRequired('** Required'), InputRequired(),))
    scheduled_return_date = DateTimeLocalField('Schedule Return', format='%Y-%m-%dT%H:%M',
                                               validators=(DataRequired('** Required'), InputRequired(),))
    submit = SubmitField('Schedule')  # OR RESERVE???


# page for individual items. gets passed the item id, returns the object.
@order_item.route('order/item/<item_id>', methods=['GET', 'POST'])
@login_required
def display_item(item_id):
    ...
    if request.method == 'POST':
        ...
        if form.scheduled_pickup_date.data < datetime.datetime.now():
            ...
            create_order(session['scheduled_pickup_date'], session['scheduled_return_date'], item_id, quantity)
            flash('Order Scheduled! Visit profile to confirm pickup.', category='success')
            return redirect(url_for('views.home'))
    return render_template('order.html', user=current_user, currentItem=current_item,
...
@order_item.route('static/date/<date_id>', methods=['GET', 'POST'])
@login_required
def date(date_id):
    # scheduled_pickup_date = session['scheduled_pickup_date']
    # scheduled_return_date = session['scheduled_return_date']
    return render_template('date.html', user=current_user, currentItem=Item.query.get(date_id))
...
def create_order(scheduled_pickup_date, scheduled_return_date, item_id, quantity):
    new_order = Order(renter_id=current_user.id, scheduled_pickup_date=scheduled_pickup_date,
                      ...
                      is_active=True,
                      quantity=quantity, )  # When is_active = True, item stays out of search- update item avail
